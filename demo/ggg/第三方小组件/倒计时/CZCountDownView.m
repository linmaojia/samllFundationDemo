...
- (void)getDetailTimeWithTimestamp:(NSInteger)timestamp{
    ...
    if (self.type == 0) {
        self.dayLabel.text = [NSString stringWithFormat:@"剩%zd时%zd分%zd秒 自动关闭",hour,minute,second];
    }
    else if (self.type == 1) {
        self.dayLabel.text = [NSString stringWithFormat:@"剩%zd天%zd时 自动收货",day,hour];
    } else if (self.type == 2) {
        self.dayLabel.text = [NSString stringWithFormat:@"卖家%zd天%zd时不发货将自动退款",day,hour];
    }
...
- (void)layoutSubviews{
    ...
    self.dayLabel.frame = CGRectMake(0, 0, viewW, labelH);
...
- (UILabel *)dayLabel{
    if (_dayLabel == nil) {
        ...
        _dayLabel.font = systemFont(12);
        _dayLabel.textColor = [UIColor whiteColor];
        _dayLabel.numberOfLines = 0;
//        _dayLabel.backgroundColor = hexColor(FF6600);
    }
    return _dayLabel;
}
...
//        _hourLabel.backgroundColor = hexColor(FF6600);
    }
    return _hourLabel;
}
...
//        _minuesLabel.backgroundColor = hexColor(FF6600);
    }
    return _minuesLabel;
}
...
//        _secondsLabel.backgroundColor = hexColor(FF6600);
    }
